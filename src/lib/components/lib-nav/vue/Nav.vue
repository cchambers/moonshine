<template>
  <div v-cloak>
    <div
      v-hammer:tap="toggleNav"
      class="show-nav nav-icon"
      :class="{ tip: !usedShortcut }"
    >
      <i class="material-icons-round" :class="{ active: !navShown }">menu</i>
      <i class="material-icons-round" :class="{ active: navShown }"
        >arrow_back</i
      >
    </div>
    <div class="lib-nav">
      <nav>
        <div ref="primary" class="primary">
          ...
          <div
            id="prod-toggle"
            v-hammer:tap="toggleFilter"
            :class="{ active: prodFilter }"
          >
            <div class="px30">
              <i v-if="prodFilter" class="material-icons-round nav-icon"
                >toggle_on</i
              >
          ...
          <ul class="nav-list">
            ...
            <li v-for="(item, key) in items" :key="item.id">
              <div class="header" :cat="key">
                {{ key }}
              </div>
              <ul>
                <li
                  v-for="(component, element) in item"
                  :key="component.id"
                  :hidden="shouldHide(component.production)"
                >
          ...
          <div v-hammer:tap="goHome" class="documentation pad-x-quarter pad-y">
            <svg
    ...
    <ul class="filtered-results">
      <li v-for="item in filteredResults" :key="item.index">
  ...
  methods: {
    ...
    handleActive() {
      const activeEl = document.querySelector(
        `nav [href$="${window.location.pathname}"]`,
      );
      if (activeEl) {
    ...
    doSearch(e) {
      ...
      if (value === '') {
        ...
        this.filteredResults = this.searchable.filter(
          item =>
